                                    ...
                                    <div class="col-auto">
                                        @if ($item['status'] != 'Đã hủy'&& $item['status'] != 'Đã giao'&& $item['status'] != 'Đang giao'&& $item['status'] != 'Đã xác nhận'&& $item['status'] != 'Đã nhận hàng')
                                            <form action="{{ route('admin.accept', ['order_id' => $order->order_id]) }}"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                        class="badge bg font-size-15 mb-3 py-3 {{ app('App\Http\Controllers\OrderController')->getOrderStatusClass($item['status']) }}"
                                                        style="font-family: Arial, sans-serif; background-color: green; border-color: black">{{ $item['button_text'] }}</button>
                                            </form>
                                        @endif
                                        ...
                                        @if ($item['status'] != 'Đã hủy' && $item['status'] != 'Đã nhận hàng')
                                            <form
                                                ...
                                                <button type="submit" style="border-radius: 40px"
                                                        class="badge bg btn btn-pill btn-primary font-size-15 mb-3 py-3">
                                                    Cập nhật đơn hàng
                                                </button>
                                            </form>
                                        @endif
